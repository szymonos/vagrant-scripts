...
if [ "$SYS_ID" = 'fedora' ]; then
  ...
  curl -Lsk "https://github.com/PowerShell/PowerShell/releases/download/v$REL/powershell_$REL-1.deb_amd64.deb" -o /tmp/powershell.deb
  apt-get install -fy /tmp/powershell.deb && rm -f /tmp/powershell.deb
else
  [ "$SYS_ID" = 'opensuse' ] && zypper in -y libicu
  curl -Lsk https://github.com/PowerShell/PowerShell/releases/download/v$REL/powershell-$REL-linux-x64.tar.gz -o /tmp/powershell.tar.gz
